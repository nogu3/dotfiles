...
| `.scripts/` | PATH に追加するユーティリティスクリプト (`gh-prw`, `ghurl`, `gwa`/`gwr` (worktree 作成/削除), `ide`, `ruby-dev-tool`, `setup-xremap`, `setup-fingerprint`, `fix-fingerprint` (ウェッジした指紋センサーを USB リセットで復旧)) |
