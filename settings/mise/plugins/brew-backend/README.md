...
MacではBrewを使用し、Linuxでは標準のmiseプラグインを使用するという統一的な管理を実現するため、`init.sh` でOSを自動判定して設定を切り替えています。
...
2.  macOS上では、`init.sh` スクリプトが実行されると、自動的に対象ツールに対してこのバックエンドプラグインがリンクされます。

    ```bash
    # init.sh 内の処理イメージ
    if [[ $OS = "MACOS" ]]; then
      # jq, ripgrep, fd などのツールに対して brew-backend をリンク
      mise plugin link jq .../settings/mise/plugins/brew-backend
    fi
    ```

3.  Linux上では、このステップはスキップされ、`mise` は標準のレジストリプラグインを使用します。
